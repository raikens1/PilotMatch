...
To try and understand this better, I want to think about a smaller sample size.

```{r}
rho <- 0.2
nu = 1
#simulate data
df <- generate_xSITA_data(N = 100, p = 10, true_mu = "X1/3-2", nu = nu, rho = rho, sigma = 1)
k = 1
...
Something buggy happened to the MDM results; they're almost exactly the same as propensity score.
```{r}
read_xSITA_data_1000 <- function(i, path_to_file, nu){
  filename <- paste(path_to_file, "xSITA_results_",i,"_10_1000_nu_", nu * 10, sep = "")
  dat <- read.csv(filename) %>%
...
# tau = 1, nu = 0.2, confounder attenuates
...
dat <- lapply(1:10, function(x) read_xSITA_data_1000(x, "../data/SITA_violations/SITA_attenuator_nu_02/", 0.2)) %>% bind_rows
```
...
# tau = 0, nu = 0.2, confounder attenuates
