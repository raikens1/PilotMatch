...
```{r}
read_data_1000 <- function(i, path_to_file, nu){
...
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/SITA_violations/nu_01/", 0.1)) %>% bind_rows
```
...
```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/SITA_violations/nu_02/", 0.2)) %>% bind_rows
```
...
```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/SITA_violations/nu_05/", 0.5)) %>% bind_rows
```
...
```{r}
read_data_10 <- function(i, path_to_file){
  filename <- paste(path_to_file, "xSITA_results_",i,"_10_10_nu_10", sep = "")
  dat <- read.csv(filename) %>%
    mutate(rho = i/10)
  return(dat)
}

dat <- lapply(1:10, function(x) read_data_10(x, "../data/SITA_violations/tuning/nu_10/")) %>% bind_rows
```
