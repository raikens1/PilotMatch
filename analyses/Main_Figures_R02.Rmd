...
NOTE: One of our reviewers asked what the mean of the vertical distances is in panel B, above.  We're taking it to mean that if $i_1$ and $i_0$ are the indices of a pair of treated and control individuals, they want to know $E\left[\Psi(X_{i_1}) - \Psi(X_{i_0})\right]$.

```{r}
match_viz_data <- function(match_formula, rho){
  df <- generate_data(N = 2000, p = 10, true_mu = "X1/3-3", rho = rho, sigma = 1)
  oracle_df <- df %>% 
    mutate(prog = rho*X1 + sqrt(1-rho^2)*X2, 
           prop = 1/(1+exp(-(mu))))
  ...
  # match
  mymatch <- pairmatch(match_formula, controls = 1, oracle_df)
...
mahal_mean_difs <- replicate(100, match_viz_data(t ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9 + X10, 0.5)) 
prog_mean_difs <- replicate(100, match_viz_data(t ~ prop + prog, 0.5))
prog_mean_difs <- replicate(100, match_viz_data(t ~ prop + prog, 0.5))
```


```{r}
difs_data <- data.frame(method = rep(c("Propensity", "Joint"), each = 100),
                        mean_difference = c(prop_mean_difs, prog_mean_difs))
