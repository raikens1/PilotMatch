...
b <- ggplot(plt_data, aes(x = rho, y = Variance,  group = method, color = method)) +
  geom_line() + geom_point() +
  xlab(expression(paste("Correlation, ", rho)))+
  theme(legend.title=element_blank())+
  scale_color_brewer(palette="Set1")
...
ggsave("../figures/Figure4.pdf", p,  width=8.5, height=2.5)
```
...
a <- ggplot(plt_data, aes(x = rho, y = Bias)) +
  geom_line() + geom_point() +
  xlab(expression(paste("Correlation, ", rho)))+
  facet_wrap(~method) +
  scale_color_brewer(palette="RdYlBu")

b <- ggplot(plt_data, aes(x = rho, y = Variance)) +
  geom_line() + geom_point() + facet_wrap(~method) +
  xlab(expression(paste("Correlation, ", rho)))+
  scale_color_brewer(palette="RdYlBu")

c <- ggplot(plt_data, aes(x = rho, y = MSE)) +
  geom_line() + geom_point() + 
  xlab(expression(paste("Correlation, ", rho)))+
  facet_wrap(~method)+
  scale_color_brewer(palette="RdYlBu")

ggarrange(a, b, c, ncol = 1, nrow = 3, common.legend = TRUE, legend = "right", labels = "AUTO")
```
...
```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/fewer_controls/mu_x1_over_3_minus_275/")) %>% bind_rows
```
...
```{r}
pdf("../figures/Figure6.pdf",  width=8.5, height=3.5)
ggarrange(a,b,c, ncol= 3, labels = "AUTO" )
dev.off()
```