...
\end{align*}
so that $\text{Cor}(\phi(X_i), \Psi(X_i)) \propto \rho$.  The propensity score formula was chosen such that there were approximately 100 treated observations in each dataset. We consider $p=10$, $\rho = 0, 0.1,\hdots, 0.9, 1.0,$ and $k=1,\hdots, 10$. Each simulation consisted of a dataset of size $n=2000$ and was repeated $N=2000$ times.
...
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/tau_half/")) %>% bind_rows
#dat2 <- lapply(1:10, function(x) read_data_1000(x, "../data/tau_half/")) %>% bind_rows
...
```{r fig.width=8.5, fig.height=6, echo = FALSE}
pdf("../figures/Figure2.pdf",  width=8.5, height=3.5)
ggarrange(a, b, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right", labels = "AUTO")
dev.off()
```
...
```{r fig.width=8.5, fig.height=6, echo = FALSE}
pdf("../figures/Figure3.pdf",  width=8.5, height=3.5)
ggarrange(c, d, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right", labels = "AUTO")
dev.off()
```
