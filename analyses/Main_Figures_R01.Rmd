...
```{r}
plt_data %>%
  filter(k == 1, method != "Mahalanobis") %>%
  select(method, rho, Standard.Deviation) %>%
  spread(method, Standard.Deviation) %>%
  mutate(Percent_change = 1 - Pilot/Propensity)

plt_data %>%
  filter(k == 2, method != "Mahalanobis") %>%
  select(method, rho, Standard.Deviation) %>%
  spread(method, Standard.Deviation) %>%
  mutate(Percent_change = 1 - Pilot/Propensity)
```
