...
This decreases the importance of $X_1$ in determining treatment assignment, while keeping the number of treated individuals at approximately 100.  Doing this decreases bias while increasing variance.  I think actually these results look great for our narrative.  I'm going to use these in the paper.
...
This decreases the importance of $X_1$ in determining treatment assignment, while keeping the number of treated individuals at approximately 100.  Doing this decreases bias while increasing variance.  It might be a happy medium between the two propensity score formulae above
...
dat <- dat %>% filter(k <=8) %>% mutate(squared_err = (estimate-true_tau)**2, k = as.factor(k))
...
# Control Reserve Size

Here's something new: what happens when there are fewer controls?
...
```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/fewer_controls/mu_x1_minus_31/")) %>% bind_rows
```
...
Here, we suppose that we use the basic simulation set-up from the paper,
...
with $p = 5$ rather than 10.

```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/tuning/p_5/")) %>% bind_rows
```
...
```{r, fig.width=8.5, fig.height=9, echo = FALSE}
a <- ggplot(plt_data, aes(x = rho, y = Bias, group = k, color = k)) +
  geom_line() + geom_point() +
  xlab(expression(paste("Correlation, ", rho)))+
  facet_wrap(~method) +
...
but let's assume that the models are correctly specified

```{r}
dat <- lapply(1:10, function(x) read_data_1000(x, "../data/tuning/correct_specification/")) %>% bind_rows
```
...
Here, we suppose that we use the basic simulation set-up from the paper, with a small tweak to the coefficient of X1 in the propensity score formula
...
Except now the score models are both fit with a lasso model.

Note that the coefficient of X1 in the propensity score model was increased because otherwise it was common for the lasso regression to over-regularize and conclude that all coefficients were zero.
