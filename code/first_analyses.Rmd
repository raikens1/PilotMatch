...
plt_data <- dat %>% 
  ...
  summarize(bias = abs(mean(estimate) - true_tau), 
            ...
            mse = bias^2 + variance) %>%
  ungroup() %>%
  mutate(method = recode(method, propensity = "Propensity", mahalanobis = "Mahalanobis", prognostic = "Buffalo"))

ggplot(plt_data, aes(x = rho, y = bias, group = k, color = k)) + geom_line() + geom_point() + facet_wrap(~method)
ggplot(plt_data, aes(x = rho, y = variance, group = k, color = k)) + geom_line() + geom_point() + facet_wrap(~method)
ggplot(plt_data, aes(x = rho, y = mse, group = k, color = k)) + geom_line() + geom_point() + facet_wrap(~method)
ggplot(plt_data, aes(x = rho, y = median_gamma, group = k, color = k)) + geom_line() + geom_point() + facet_wrap(~method)
```
