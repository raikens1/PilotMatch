...
run_sim <- function(rho = 0.1, p = 10, nsim = 10, out_file = "test_", true_mu = "X1/3 - 3", ks = 1:10, sigma = 1, tau = 0.5) {
  t1 <- proc.time()
  # simulate
  results <- replicate(nsim, simulate(generate_data(rho=rho, p = p, true_mu = true_mu, sigma = sigma, tau = tau),
                                      verbose = TRUE, ks = ks),
                       simplify = FALSE) %>% 
    bind_rows()
